 ...
 * @Package：cn.ucaner.skeleton.opencv.common.utils.gray
 * @Description： <p> GrayUtils 对图像灰度化处理的工具类 默认有效数据的灰度值小于底色,即有效数据更清晰 </p>
 * @Author： - Jason
 * @CreatTime：2019/8/20 - 14:46
 * @Modify By：
...
public class GrayUtils {
    ...
    public static Mat grayColByMidle(Mat src){
        ...
        List<List<Double>> data = OpenCvMathUtils.MatPixelToList(src);
    ...
    public static Mat grayColByMidle(Mat src , List<List<Double>> data){
        ...
        for(int j = 0 ; j < data.size() ; j++){
            ...
            int avg = (int)((OpenCvMathUtils.getSumInList(list) / list.size()) * 0.95);
            //随机的更新像素值
    ...
    public static Mat grayColByKLargest(Mat src , int k , List<List<Double>> data){
        ...
        for(int j = 0 ; j < data.size() ; j++){
            ...
            double d = OpenCvMathUtils.findKthLargest(doubles1, list.size() / k);
            //随机的更新像素值
